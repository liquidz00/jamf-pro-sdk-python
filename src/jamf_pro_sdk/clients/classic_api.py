...
class ClassicApi:
    ...
    def update_category_by_id(self, category: CategoryId, data: str | ClassicCategory) -> None:
    ...
    def create_category(self, data: str | ClassicCategory) -> int:
    ...
    def update_computer_by_id(self, computer: ComputerId, data: str | ClassicComputer) -> None:
    ...
    def create_computer_group(self, data: str | ClassicComputerGroup) -> int:
    ...
    def create_advanced_computer_search(self, data: str | ClassicAdvancedComputerSearch) -> int:
    ...
    def create_package(self, data: str | ClassicPackage) -> int:
...
class AsyncClassicApi:
    """Provides an async curated interface to the Jamf Pro Classic API."""
    ...
    async def get_category_by_id(self, category: CategoryId) -> ClassicCategory:
        ...
        """
        category_id = AsyncClassicApi._parse_id(category)
    ...
    async def get_computer_by_id(
        ...
        """
        computer_id = AsyncClassicApi._parse_id(computer)
        if subsets:
            if not all(i.lower() in VALID_COMPUTER_SUBSETS for i in subsets):
                raise ValueError(f"Invalid subset(s). Must be one of: {VALID_COMPUTER_SUBSETS}.")
            path = f"computers/id/{computer_id}/subset/{'&'.join(subsets)}"
    ...
    async def get_computers(
        self, computers: list[ComputerId] = None, subsets: Iterable[str] = None
    ) -> AsyncIterator[ClassicComputer]:
        """Returns all requested computer records by their IDs. This is a wrapper to the concurrent
        API requests method. If ``computers`` is not provided a call to
        :meth:`~jamf_pro_sdk.clients.classic_api.AsyncClassicApi.list_all_computers` is made to obtain
        the full list of computer IDs.
        ...
        :return: list of computers.
        :rtype: AsyncIterator[~jamf_pro_sdk.models.classic.computers.ClassicComputer]

        """
        if not computers:
            computers = await self.list_all_computers()

        async for result in self.concurrent_api_requests(
            self.get_computer_by_id, [{"computer": i, "subsets": subsets} for i in computers]
        ):
            yield result
    ...
    async def get_advanced_computer_search_by_id(self, advanced_search: AdvancedComputerSearchId):
        """Returns a single advanced computer search using the ID.
        ...
        """
        advanced_search_id = AsyncClassicApi._parse_id(advanced_search)
        resp = await self.api_request(
            method="get", resource_path=f"advancedcomputersearches/id/{advanced_search_id}"
        )
        return ClassicAdvancedComputerSearch(**resp.json()["advanced_computer_search"])
    ...
    async def update_advanced_computer_search_by_id(
        ...
        :param return_updated: If ``True`` the :meth:`~jamf_pro_sdk.clients.classic_api.AsyncClassicApi.get_advanced_computer_search_by_id`
            operation will be called after the update operation to return the updated results.
        :type return_updated: bool
    ...
    async def create_package(self, data: str | ClassicPackage) -> int:
        ...
        Only the ``name`` and ``filename`` are required.
        Use with :meth:`~jamf_pro_sdk.clients.pro_api.AsyncProApi.create_jcds_file_v1` to upload the
        package file to a Jamf Cloud Distribution Point.
    ...
    async def get_package_by_id(self, package: PackageId) -> ClassicPackage:
        ...
        """
        package_id = AsyncClassicApi._parse_id(package)
